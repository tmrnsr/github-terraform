terraform {
  required_providers {
    ...
    backend "azurerm" {
    resource_group_name  = "rg-backend-trs-state"
    storage_account_name = "storagebacktfspx595m8gak"
    container_name       = "tfstate-trs"
    key                  = "backend.terraform.tfstate-testweb"
  }
  }
...
resource "azurerm_resource_group" "rg_backend_trs" {
  name     = var.rg_backend_trs
...
resource "azurerm_storage_account" "sa_backend" {
  ...
  resource_group_name      = azurerm_resource_group.rg_backend_trs.name
  location                 = azurerm_resource_group.rg_backend_trs.location
